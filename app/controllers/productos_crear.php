...
try {
    ...
    // Validar código de material si se proporciona
    if (!empty($datos['material_code'])) {
        $productoExistente = $model->obtenerProductoPorMaterialCode($datos['material_code']);
        if ($productoExistente) {
            echo json_encode([
                'success' => false,
                'message' => 'Ya existe un producto con este código de material'
            ]);
            exit;
        }
    }
    
    $id = $model->crearProducto($datos);
    ...
    error_log("Error SQL al crear producto: " . $e->getMessage());
    http_response_code(500);
    echo json_encode([
        'success' => false,
        'message' => 'Error de base de datos al crear el producto',
        'error' => $e->getMessage()
    ]);
} catch (Exception $e) {
