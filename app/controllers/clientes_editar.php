...
$clienteExistente = $model->obtenerClientePorId($id);
if (!$clienteExistente) {
...
if (empty($errores)) {
    $clienteConRFC = $model->obtenerClientePorRFC($rfc, $id);
    if ($clienteConRFC) {
        $errores[] = 'Ya existe otro cliente con ese RFC';
    }
}
...
try {
    ...
    $model->actualizarCliente($id, $datos);
